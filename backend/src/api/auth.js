...
router.post('/register', async (req, res) => {
  try {
    ...
    return res.status(201).json({
      token,
      tenant: { id: tenant.id, subdomain: tenant.subdomain, name: tenant.name, plan: tenant.plan },
...
router.post('/login', async (req, res) => {
  try {
    ...
    const tenant = await prisma.tenant.findUnique({
      where: { subdomain },
      include: { users: { where: { email, isActive: true } } }
    });
...
router.post('/verify', async (req, res) => {
  try {
    ...
    return res.json({ valid: true, userId: decoded.userId, tenantId: decoded.tenantId });
